...
from qstack.stabilizers import Context as StabilizerContext, gadget_with_error_correction, update_syndrome_bit
...
class Context(StabilizerContext):
    def __init__(self, qubit_count: int, register_count: int):
        super().__init__(qubit_count=qubit_count * 7, register_count=register_count, distance=1)
        self.blocks = {QubitId(i): [QubitId(i * 7 + j) for j in range(7)] for i in range(qubit_count)}
...
def measure_z(inst: Instruction, context: Context) -> Circuit:
    ...
    def measure_decoder(memory: list[int], corrections):
        ...
        updated_value = update_syndrome_bit(value, z_op, last_error)
