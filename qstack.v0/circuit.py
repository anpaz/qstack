...
from ..qcir.utils import cache_field
...
@dataclass(frozen=True)
class Circuit:
    instructions: list[Comment | Tick | GadgetDefinition]
...
def circuit_metadata(circuit: Circuit) -> dict[str, str]:
    ...
    for inst in circuit.instructions:
        if isinstance(inst, Comment):
            pass
        elif isinstance(inst, Tick) or isinstance(inst, GadgetDefinition):
            break
...
def circuit_dimensions(circuit: Circuit) -> tuple[int, int, int, int]:
    ...
    for i in circuit.instructions:
        if isinstance(i, GadgetDefinition):
            instr_count += 1
            for t in i.targets:
                if t.value > max_q:
                    max_q = t.value
    ...
    return (q_count, instr_count, steps)
