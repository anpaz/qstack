...
def compile(gadget: Gadget) -> Gadget:
    ...
    def repetition_code_decoder(memory: tuple[bool]) -> int:
        memory = list(memory)
        correction = [qstack.paulis.I] * qubit_count * 3
        for g in compiled_gadgets:
    ...
    return Gadget(gadget.name, new_circuit, decoder=repetition_code_decoder)
